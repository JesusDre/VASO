                    ...
                    <div style={cardStyle}>
                        ...
                        <form onSubmit={subirImagen}>
                            <div style={{ marginBottom: 12 }}>
                                <label htmlFor="img_tipo" style={labelStyle}>Tipo de imagen</label>
                                <select id="img_tipo" value={imgTipo} onChange={(e) => setImgTipo(e.target.value)} style={selectStyle}>
                            ...
                            <div style={{ marginBottom: 12 }}>
                                <label htmlFor="img_desc" style={labelStyle}>Descripción</label>
                                <input id="img_desc" type="text" value={imgDesc} onChange={(e) => setImgDesc(e.target.value)} placeholder="Ej: Bosque nocturno" style={inputStyle} />
                            </div>
                            <div style={{ marginBottom: 16 }}>
                                <label htmlFor="img_file" style={labelStyle}>Archivo (PNG, JPG, WebP)</label>
                                <input id="img_file" type="file" accept="image/*" onChange={(e) => setImgFile(e.target.files[0])} style={{ ...inputStyle, padding: '6px 10px' }} />
                            </div>
                                ...
                                <>
                                    <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
                                        {FILTROS.map(({ key, label, color }) => {
                                            ...
                                            const bgActivo = color ? `${color}18` : 'var(--accent-light)';
                                            const bgColor = activo ? bgActivo : 'var(--surface)';
                                            return (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                        ...
                                        {imgsFiltradas.map((img) => {
                                            const srcFromUrl = img.url ? `${API_BASE}${img.url}` : null;
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20 }}>
                    <div style={cardStyle}>
                        ...
                        <form onSubmit={subirAudio}>
                            <div style={{ marginBottom: 12 }}>
                                <label htmlFor="aud_desc" style={labelStyle}>Descripción</label>
                                <input id="aud_desc" type="text" value={audDesc} onChange={(e) => setAudDesc(e.target.value)} placeholder="Ej: Música de tensión" style={inputStyle} />
                            </div>
                            <div style={{ marginBottom: 16 }}>
                                <label htmlFor="aud_file" style={labelStyle}>Archivo (MP3, OGG, WAV)</label>
                                <input id="aud_file" type="file" accept="audio/*" onChange={(e) => setAudFile(e.target.files[0])} style={{ ...inputStyle, padding: '6px 10px' }} />
                            </div>
                            <button type="submit" disabled={subiendo} style={{ ...btnPrimary, width: '100%' }}>
            ...
            {modalEditar && (
                <Modal isOpen titulo={`Editar ${modalEditar.tipo === 'imagen' ? 'imagen' : 'audio'}`} onClose={() => !guardandoEdit && setModalEditar(null)} ancho={420}>
                    <form onSubmit={confirmarEditar} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        <div>
                            <label htmlFor="edit_descripcion" style={labelStyle}>Descripción</label>
                            <input id="edit_descripcion" type="text" value={editForm.descripcion} onChange={(e) => setEditForm({ ...editForm, descripcion: e.target.value })}
                                placeholder="Descripción del recurso" style={inputStyle} autoFocus />
                        </div>
                        {modalEditar.tipo === 'imagen' && (
                            <div>
                                <label htmlFor="edit_tipo_imagen" style={labelStyle}>Tipo de imagen</label>
                                <select id="edit_tipo_imagen" value={editForm.tipo} onChange={(e) => setEditForm({ ...editForm, tipo: e.target.value })} style={selectStyle}>
                                    <option value="escenario">Fondo de escena</option>
                                    <option value="personaje">Sprite de personaje</option>
                                    <option value="portada">Portada de historia</option>
                                </select>
