...
export default function TabRoles() {
    ...
    let textoBoton;
    if (guardando) textoBoton = '...';
    else if (editandoId) textoBoton = 'Actualizar';
    else textoBoton = 'Crear';
        ...
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            <div style={{ width: 280, flexShrink: 0, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 20, boxShadow: 'var(--shadow-sm)' }}>
                ...
                <form onSubmit={handleGuardar} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div>
                        <label className="nv-label" htmlFor="rol-nombre">Nombre del rol</label>
                        <input
                            id="rol-nombre"
            ...
            <div style={{ flex: 1, minWidth: 240 }}>
                {(() => {
                    if (cargando) return <div style={{ textAlign: 'center', padding: '2rem 0' }}><Spinner /></div>;
                    if (roles.length === 0) return <Empty texto="Sin roles configurados." />;
                    return (
